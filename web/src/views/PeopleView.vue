...
async function faceSample(p:Person,e:Event){const input=e.target as HTMLInputElement,f=input.files?.[0];if(!f)return;busy.value=`face-${p.id}`;error.value='';notice.value='';try{await uploadFaceSample(p.id,f);notice.value=t('product.people.faceSampleAdded',{name:p.name});await Promise.all([load(),loadDetail(p.id)]);const next=new Set(expanded.value);next.add(p.id);expanded.value=next}catch(reason){error.value=errText(reason)}finally{busy.value='';input.value=''}}
...
async function removeFaceSample(p:Person,s:FaceSample){const ok=await dialog.showConfirm({title:t('product.people.deleteFaceSampleTitle'),message:t('product.people.deleteFaceSampleConfirm',{name:p.name}),detail:t('product.people.deleteFaceSampleImpact'),confirmText:t('product.people.deleteSample'),cancelText:t('btn.cancel'),danger:true});if(!ok)return;busy.value=s.id;error.value='';notice.value='';try{await deleteFaceSample(p.id,s.id);notice.value=t('product.people.faceSampleDeleted');await Promise.all([load(),loadDetail(p.id)])}catch(reason){error.value=reason instanceof Error?reason.message:String(reason)}finally{busy.value=''}}
async function togglePreview(p:Person,s:VoiceSample){
  const player=samplePlayer.value;if(!player)return
  if(previewId.value===s.id&&previewUrl.value){
    if(player.paused){try{await player.play()}catch(reason){error.value=reason instanceof Error?reason.message:String(reason);stopPreview()}}else player.pause()
    return
  }
  previewLoading.value=s.id;stopPreview();const token=++previewToken
  try{
    const url=await getVoiceSampleAudioUrl(p.id,s.id);if(token!==previewToken){revokeObjectUrl(url);return}
    previewUrl.value=url;previewId.value=s.id;await nextTick();samplePlayer.value?.load();await samplePlayer.value?.play()
  }catch(reason){if(token===previewToken)error.value=reason instanceof Error?reason.message:String(reason);stopPreview()}finally{previewLoading.value=''}
}
function revokeObjectUrl(url:string){if(url.startsWith('blob:'))URL.revokeObjectURL(url)}
function stopPreview(){const player=samplePlayer.value;player?.pause();if(player){player.removeAttribute('src');player.load()}if(previewUrl.value)revokeObjectUrl(previewUrl.value);previewUrl.value='';previewId.value='';previewPlaying.value=false;previewToken++}
...
<template><section class="product-page"><div class="page-head"><div><div class="eyebrow">{{t('product.people.eyebrow')}}</div><h1>{{t('product.people.title')}}</h1><p>{{t('product.people.lead')}}</p></div></div>
  ...
  <div v-if="loading" class="state-card">{{t('common.loading')}}</div><div v-else class="people-grid">
    <article v-for="p in filtered" :key="p.id" :class="{expanded:expanded.has(p.id)}">
      ...
      <div class="person-copy"><h3>{{p.name}}</h3><p>{{p.sample_count?t('product.people.samples',{count:p.sample_count,seconds:(p.total_sample_duration??0).toFixed(1)}):t('product.people.noSamples')}} · {{t('product.people.faceSamples',{count:p.face_sample_count||0})}}</p></div>
      ...
      <div class="sample-actions">
        ...
        <label class="sample face"><input type="file" accept="image/*,.jpg,.jpeg,.png,.webp,.bmp" @change="faceSample(p,$event)">{{busy===`face-${p.id}`?t('product.people.processing'):t('product.people.addFaceSample')}}</label>
      ...
      <section v-if="recId===p.id&&recState!=='idle'" class="recorder-panel">
        ...
        <div class="face-sample-section">
          <header><div><h4>{{t('product.people.faceSampleLibrary')}}</h4><p>{{t('product.people.faceSampleLibraryHint')}}</p></div><span>{{details[p.id]?.face_samples?.length??0}}</span></header>
          <div v-if="!details[p.id]?.face_samples?.length" class="sample-empty">{{t('product.people.faceSampleEmpty')}}</div>
          <div v-else class="face-sample-list">
            <div v-for="(s,index) in details[p.id].face_samples" :key="s.id" class="face-sample-row">
              <img :src="getFaceSampleImageUrl(p.id,s.id)" :alt="t('product.people.faceSampleName',{number:index+1})">
              <div class="sample-copy"><b>{{t('product.people.faceSampleName',{number:index+1})}}</b><p>{{t('product.people.faceSampleMeta',{score:Math.round((s.detection_score||0)*100),date:dateLabel(s.created_at)})}}</p><code :title="s.model_name">{{s.model_name}}</code></div>
              <button class="remove-sample" :disabled="busy===s.id" @click="removeFaceSample(p,s)">{{busy===s.id?t('product.people.deleting'):t('product.people.deleteSample')}}</button>
            </div>
          </div>
        </div>
      </section>
...
.sample-actions{display:flex;gap:8px}.sample.record{color:var(--teal)}.sample.face{color:var(--gold)}.sample.record:disabled{opacity:.45;cursor:not-allowed}
...
.face-sample-section{margin-top:18px;padding-top:18px;border-top:1px solid var(--border-soft)}.face-sample-list{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:8px}.face-sample-row{display:grid;grid-template-columns:56px minmax(0,1fr) auto;align-items:center;gap:10px;padding:10px;background:var(--ink-3);border:1px solid var(--border-soft);border-radius:8px}.face-sample-row img{width:56px;height:56px;object-fit:cover;border-radius:6px;background:var(--ink-1);border:1px solid var(--border-soft)}
