...
_ACTIVE_PROGRESS_TOTAL_BYTES: int | None = None


def _make_progress_tqdm_class():
    from tqdm.auto import tqdm as base_tqdm
    from app.services.download_progress import update_progress

    class DownloadProgressTqdm(base_tqdm):
        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
            self._progress_key = _ACTIVE_PROGRESS_KEY
            self._progress_total_bytes = _ACTIVE_PROGRESS_TOTAL_BYTES
            self._progress_is_bytes = (
                self._progress_key is not None
                and isinstance(self.total, (int, float))
                and int(self.total) >= 10_000_000
            )
        ...
        def _publish_progress(self) -> None:
            if not self._progress_is_bytes or not self._progress_key:
                return
            total = self._progress_total_bytes or int(self.total or 0)
            update_progress(
                self._progress_key,
                downloaded_bytes=int(self.n or 0),
                total_bytes=total,
                status="downloading",
                message=str(getattr(self, "desc", "") or "").strip() or None,
            )
...
def _latest_staging_dir(dst: str) -> str | None:
    parent = os.path.dirname(dst)
    prefix = f".{os.path.basename(dst)}.partial-"
    try:
        candidates = [
...
def resolve_hf(
    ...
    *,
    ...
    staging = _latest_staging_dir(dst) or _new_staging_dir(dst)
    published = False
    logger.info("[MODELS] HF staging: %s", staging)
    global _ACTIVE_PROGRESS_KEY, _ACTIVE_PROGRESS_TOTAL_BYTES
    old_progress_key = _ACTIVE_PROGRESS_KEY
    old_progress_total = _ACTIVE_PROGRESS_TOTAL_BYTES
    try:
        if progress_key:
            from app.services.download_progress import start_progress

            start_progress(
                progress_key,
                label=progress_label or name,
                total_bytes=progress_total_bytes,
            )
            _ACTIVE_PROGRESS_KEY = progress_key
            _ACTIVE_PROGRESS_TOTAL_BYTES = progress_total_bytes
        kwargs = {}
        if progress_key:
            kwargs["tqdm_class"] = _make_progress_tqdm_class()
        attempts = int(os.environ.get("HF_DOWNLOAD_ATTEMPTS", "5"))
        for attempt in range(1, max(1, attempts) + 1):
            try:
                snapshot_download(repo_id, revision=revision, local_dir=staging, **kwargs)
                break
            except Exception as exc:
                if attempt >= max(1, attempts):
                    raise
                logger.warning(
                    "[MODELS] HF download failed, retrying %s/%s: %s",
                    attempt,
                    attempts,
                    exc,
                )
                if progress_key:
                    ...
                    update_progress(
                        progress_key,
                        status="retrying",
                        message=f"retry {attempt}/{attempts}",
                    )
        ...
        published = True
        if progress_key:
            from app.services.download_progress import finish_progress

            finish_progress(progress_key)
    except Exception as exc:
        if progress_key:
            from app.services.download_progress import fail_progress

            fail_progress(progress_key, str(exc))
        raise
    finally:
        _ACTIVE_PROGRESS_KEY = old_progress_key
        _ACTIVE_PROGRESS_TOTAL_BYTES = old_progress_total
        if published:
            shutil.rmtree(staging, ignore_errors=True)
    return dst
